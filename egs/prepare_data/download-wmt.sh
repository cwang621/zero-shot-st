...
export version="wmt14"
export target=de
export WMT_DATA_ROOT=~/data/WMT
while [[ "$#" -gt 0 ]]; do
    case $1 in
        --data-dir) export WMT_DATA_ROOT="$2"; shift ;;
...
echo "arguments"
echo "WMT_DATA_ROOT: $WMT_DATA_ROOT"
...
if [[ "$target" == "ro" ]]; then
    pair_dir=SETIMES/en-$tgt
    mkdir -p $pair_dir
    grep '<s id' SETIMES/raw/en/setimes.en-$tgt.xml | \
      sed -e 's/<s id="[0-9]*">\s*//g' | \
      sed -e 's/\s*<\/s>\s*//g' | \
      sed -e "s/\’/\'/g" > $pair_dir/setimes.en-$tgt.en
    grep '<s id' SETIMES/raw/$tgt/setimes.en-$tgt.xml | \
      sed -e 's/<s id="[0-9]*">\s*//g' | \
      sed -e 's/\s*<\/s>\s*//g' | \
      sed -e "s/\’/\'/g" > $pair_dir/setimes.en-$tgt.$tgt
fi
