...
from fairseq.data.audio.audio_utils import get_fbank, update_sample_rate, _sox_convert, convert_to_mono

logger = logging.getLogger(__name__)

class W2TDataConfig(object):
    ...
    def tgt_bpe_tokenizer(self) -> Dict:
        ...
        the other items providing the tokenizer-specific arguments.
        Tokenizers are defined in `fairseq.data.encoders.*`"""
        return self.config.get("tgt_bpe_tokenizer", {"bpe": None})
...
def get_waveform(
    ...
    if mp3_input:
        try:
            ...
            raise ImportError("Please install torchaudio")
        waveform, sample_rate = torchaudio.load(path_or_fp)
        waveform = waveform.data.numpy()
    else:
        try:
            import soundfile as sf
        except ImportError:
            raise ImportError("Please install soundfile to load WAV/FLACC/OGG audios")
...
def get_features_or_waveform(path: str, need_waveform=False, sample_rate=16000):
    ...
    """
    assert need_waveform, "Only support waveform input format"
    _path, *extra = path.split(":")
    if not op.exists(_path):
        raise FileNotFoundError(f"File not found: {_path}")

    if len(extra) == 0:
        return get_waveform(_path, output_sample_rate=sample_rate)[0]
    elif len(extra) == 2:
        if _path.endswith(".zip"):
            ...
            waveform, sample_rate = get_waveform(_path, start=int(extra[0]), frames=int(extra[1]))
            return waveform
    else:
...
class WaveToTextDataset(FairseqDataset):
    ...
    def __init__(
        ...
        if self.src_dict is not None:
            self.src_len = self.get_src_lens_and_check_oov()
        self.tgt_len = self.get_tgt_lens_and_check_oov()
    ...
    def get_src_lens_and_check_oov(self):
        ...
        for i in range(self.n_samples):
            tokenized = self.tokenize_text(self.src_texts[i], is_src=True).split(" ")
            oov_tokens = [
                t
                for t in tokenized
    ...
    def get_tgt_lens_and_check_oov(self):
        ...
        n_tokens, n_oov_tokens = 0, 0
        for i in range(self.n_samples):
            tokenized = self.tokenize_text(self.tgt_texts[i]).split(" ")
            oov_tokens = [
                t
                for t in tokenized
...
class WaveToTextDatasetCreator(object):
    # mandatory columns
